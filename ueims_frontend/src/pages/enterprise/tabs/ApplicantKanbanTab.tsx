...
// TYPES
// ============================================================
type ApplicationStatus = 'PENDING' | 'INTERVIEWING' | 'PASSED' | 'REJECTED';
...
const MOCK_APPLICANTS: ApplicantCard[] = [
  {
    id: 'a-001', assignmentId: 'asgn-001',
    studentName: 'Nguyen Van A', studentCode: 'SE161234', email: 'nguyenvana@student.fpt.edu.vn',
    major: 'Software Engineering', gpa: 3.8, jobTitle: 'Frontend Developer',
    avatarInitials: 'NVA', avatarColor: '#E67E22',
    appliedAt: '2026-06-01T10:00:00Z', status: 'PENDING',
  },
  {
    id: 'a-002', assignmentId: 'asgn-002',
    studentName: 'Tran Thi B', studentCode: 'IA162345', email: 'tranthib@student.fpt.edu.vn',
    major: 'Info Assurance', gpa: 3.4, jobTitle: 'Security Analyst',
    avatarInitials: 'TTB', avatarColor: '#3B82F6',
    appliedAt: '2026-06-02T14:30:00Z', status: 'PENDING',
  },
  {
    id: 'a-003', assignmentId: 'asgn-003',
    studentName: 'Le Van C', studentCode: 'SE163456', email: 'levanc@student.fpt.edu.vn',
    major: 'Software Engineering', gpa: 3.1, jobTitle: 'Backend Developer',
    avatarInitials: 'LVC', avatarColor: '#22c55e',
    appliedAt: '2026-06-03T09:15:00Z', status: 'INTERVIEWING',
    interviewDate: '2026-06-20T10:00:00Z', interviewLink: 'https://meet.example.com/abc123',
  },
  {
    id: 'a-004', assignmentId: 'asgn-004',
    studentName: 'Bui Van G', studentCode: 'GD162111', email: 'buivang@student.fpt.edu.vn',
    major: 'Graphic Design', gpa: 3.6, jobTitle: 'UI/UX Designer',
    avatarInitials: 'BVG', avatarColor: '#8b5cf6',
    appliedAt: '2026-06-03T16:00:00Z', status: 'INTERVIEWING',
    interviewDate: '2026-06-22T14:00:00Z',
  },
  {
    id: 'a-005', assignmentId: 'asgn-005',
    studentName: 'Pham Thi D', studentCode: 'AI162789', email: 'phamthid@student.fpt.edu.vn',
    major: 'Artificial Intelligence', gpa: 3.9, jobTitle: 'ML Engineer',
    avatarInitials: 'PTD', avatarColor: '#f59e0b',
    appliedAt: '2026-05-28T11:00:00Z', status: 'PASSED',
  },
  {
    id: 'a-006', assignmentId: 'asgn-006',
    studentName: 'Hoang Van E', studentCode: 'SE164001', email: 'hoangvane@student.fpt.edu.vn',
    major: 'Software Engineering', gpa: 2.9, jobTitle: 'QA Engineer',
    avatarInitials: 'HVE', avatarColor: '#ef4444',
    appliedAt: '2026-05-25T08:00:00Z', status: 'REJECTED',
  },
  {
    id: 'a-007', assignmentId: 'asgn-007',
    studentName: 'Dao Thi F', studentCode: 'IA163222', email: 'daothif@student.fpt.edu.vn',
    major: 'Info Assurance', gpa: 3.2, jobTitle: 'DevSecOps Engineer',
    avatarInitials: 'DTF', avatarColor: '#3b82f6',
    appliedAt: '2026-06-05T13:00:00Z', status: 'PENDING',
  },
  {
    id: 'a-008', assignmentId: 'asgn-008',
    studentName: 'Vo Thi H', studentCode: 'GD163333', email: 'vothih@student.fpt.edu.vn',
    major: 'Graphic Design', gpa: 3.5, jobTitle: 'Motion Designer',
    avatarInitials: 'VTH', avatarColor: '#22c55e',
    appliedAt: '2026-06-04T10:30:00Z', status: 'PASSED',
  },
];
...
const COLUMNS: { id: ApplicationStatus; label: string; color: string; bg: string }[] = [
  ...
  { id: 'INTERVIEWING', label: 'Interviewing', color: c.info, bg: hexToRgba(c.info, 0.06) },
  { id: 'PASSED', label: 'Passed', color: c.success, bg: hexToRgba(c.success, 0.06) },
...
const StatusBadge: React.FC<{ status: ApplicationStatus }> = ({ status }) => {
  ...
  return (
    <span style={{
      ...
      {status === 'PASSED' && <CheckCircleOutlined style={{ fontSize: 10 }} />}
      {status === 'REJECTED' && <CloseCircleOutlined style={{ fontSize: 10 }} />}
      {status === 'INTERVIEWING' && <ClockCircleOutlined style={{ fontSize: 10 }} />}
      ...
      >
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 11, color: c.textMuted }}>
            ...
            <span style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{applicant.email}</span>
          </div>
...
// ============================================================
// DRAG OVERLAY CARD (clone while dragging)
// ============================================================
...
const InterviewModal: React.FC<{
  ...
  return (
    <Modal
      ...
      width={440}
      styles={{
        content: { borderRadius: 24, padding: '24px 28px' },
    ...
    >
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        {error && (
          <div style={{ padding: '10px 14px', borderRadius: 12, background: c.errorMuted, border: `1px solid ${c.error}30`, color: c.error, fontSize: 13, fontWeight: 600 }}>
            {error}
          </div>
        )}
        ...
        <div>
          ...
          <DatePicker
            ...
            style={{ width: '100%', borderRadius: 12, borderColor: c.border }}
        ...
        <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end', paddingTop: 4 }}>
          <button
            onClick={onClose}
            style={{ padding: '10px 20px', borderRadius: 12, border: `1px solid ${c.border}`, background: '#fff', color: c.text, fontWeight: 600, cursor: 'pointer', fontSize: 13 }}
          >
            Cancel
          </button>
          <button
            ...
            style={{
              ...
              background: c.brand, color: '#fff', fontWeight: 700, cursor: submitting ? 'not-allowed' : 'pointer',
              fontSize: 13, opacity: submitting ? 0.7 : 1, boxShadow: '0 8px 22px rgba(230,126,34,0.22)',
            }}
          >
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
          <div style={{
            ...
            background: avatarColor.bg,
            display: 'flex', alignItems: 'center', justifyContent: 'center',
            color: avatarColor.text, fontSize: 22, fontWeight: 800,
            boxShadow: `0 8px 18px ${avatarColor.text}20`,
          }}>
            {applicant.avatarInitials}
          </div>
          <div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          {[
            ...
            { label: 'GPA', value: applicant.gpa.toString() },
        ...
        <div style={{ padding: '12px 14px', borderRadius: 12, background: c.bgLight, border: `1px solid ${c.borderSubtle}` }}>
          ...
          <div style={{ fontSize: 13, fontWeight: 600, color: c.text }}>{applicant.email}</div>
        </div>
        ...
        {applicant.interviewDate && (
          <div style={{ padding: '12px 14px', borderRadius: 12, background: hexToRgba(c.info, 0.06), border: `1px solid ${hexToRgba(c.info, 0.2)}` }}>
            ...
            <div style={{ fontSize: 14, fontWeight: 600, color: c.text }}>
              {dayjs(applicant.interviewDate).format('dddd, MMMM D, YYYY [at] HH:mm')}
            </div>
            {applicant.interviewLink && (
              <a href={applicant.interviewLink} target="_blank" rel="noopener noreferrer" style={{ fontSize: 12, color: c.info, marginTop: 4, display: 'block' }}>
                Join Meeting Link →
              </a>
            )}
          </div>
...
const KanbanColumn: React.FC<{
  ...
  <div style={{ flex: '0 0 280px', display: 'flex', flexDirection: 'column', minWidth: 0 }}>
    ...
    <div style={{
      ...
      border: `1px solid ${hexToRgba(column.color, 0.2)}`,
      marginBottom: 12,
      display: 'flex',
      alignItems: 'center',
      ...
      <span style={{
        ...
        background: hexToRgba(column.color, 0.15),
        color: column.color, fontSize: 12, fontWeight: 700,
      }}>
        {applicants.length}
      </span>
    </div>
...
export const ApplicantKanbanTab: React.FC = () => {
    ...
    const fetchApplicants = async () => {
      try {
        ...
        if (Array.isArray(data) && data.length > 0) {
          const mapped: ApplicantCard[] = data.map((item: any, idx: number) => ({
            id: `api-${item.assignmentId}`,
            assignmentId: item.assignmentId ?? item.id,
            studentName: item.student?.fullName ?? item.studentName ?? `Student ${idx + 1}`,
            studentCode: item.student?.studentCode ?? item.studentCode ?? 'N/A',
            ...
            avatarInitials: (item.student?.fullName ?? 'ST').substring(0, 2).toUpperCase(),
            avatarColor: AVATAR_COLORS[idx % AVATAR_COLORS.length].text,
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    // Handle INTERVIEWING: open schedule modal
  ...
  const updateStatus = async (applicant: ApplicantCard, newStatus: ApplicationStatus) => {
    try {
      await EnterpriseAssignmentService.update(applicant.assignmentId, { status: newStatus });
    } catch {
  ...
  const handleScheduleInterview = async (date: string, link: string) => {
    ...
    try {
      await EnterpriseAssignmentService.update(interviewModal.applicant.assignmentId, {
        status: 'INTERVIEWING',
        interviewDate: date,
        interviewLink: link,
      });
    ...
    setApplicants(prev =>
      prev.map(a =>
        a.id === interviewModal.applicant!.id
          ? { ...a, status: 'INTERVIEWING' as ApplicationStatus, interviewDate: date, interviewLink: link }
          : a
      )
    );
        ...
        >
          {COLUMNS.map(column => (
            <KanbanColumn
              ...
              onViewDetails={(a) => setDetailModal({ open: true, applicant: a })}
            />
          ))}

          <DragOverlay>
            {activeApplicant ? <DragCard applicant={activeApplicant} /> : null}
          </DragOverlay>
        </DndContext>
      </div>
