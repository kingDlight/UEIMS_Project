...
export const InternshipPlanApprovalsTab: React.FC = () => {
  ...
  const fetchPlans = async () => {
    ...
    try {
      ...
      message.error(err?.response?.data?.message ?? 'Failed to load pending training plans.');
      setPlans([]);
    } finally {
      setLoading(false);
    }
  ...
  const handleApprove = async (planId: string) => {
    try {
      ...
      message.success('Training plan approved successfully.');
      fetchPlans();
    } catch (err: any) {
  ...
  const handleReject = async () => {
    ...
    try {
      ...
      message.success('Training plan rejected.');
  ...
  const columns = [
    ...
    },
    {
      ...
      ellipsis: true,
      render: (text: string) => (
        <span className="text-slate-500 text-sm truncate max-w-xs">{text}</span>
      ),
    },
    {
      title: 'Status',
      ...
      render: (_: any, record: any) => (
        <div className="flex gap-2">
          ...
          <Button
            ...
            danger
            icon={<CloseOutlined />}
            onClick={() => setRejectPlanId(record.planId)}
          >
            Reject
          </Button>
        </div>
  ...
  return (
    <div className="p-6">
      <div className="mb-6 flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-slate-800">Training Plan Approvals</h1>
          <p className="text-sm text-slate-500 mt-1">
            Review and approve training plans submitted by enterprises. One plan per enterprise
            per semester.
          </p>
        </div>
      ...
      <Modal
        ...
        footer={[
          <Button key="close" onClick={() => setViewingPlan(null)}>
            Close
          </Button>,
          <Button
            key="approve"
          ...
          <div className="mt-4">
            <div className="mb-4 grid grid-cols-2 gap-4">
              <div>
                ...
                <div className="text-base text-slate-800">{viewingPlan.enterpriseName}</div>
              </div>
              <div>
                <div className="text-xs text-slate-500 font-bold uppercase">Semester</div>
            ...
            <div className="mb-6 p-4 bg-slate-50 rounded-lg border border-slate-200">
              ...
              <div className="text-sm text-slate-700 whitespace-pre-wrap">
                {viewingPlan.overallGoal || 'No goal specified'}
              </div>
            </div>
                  ...
                  {viewingPlan.tasks?.map((task: any) => (
                    <tr key={task.planItemId} className="bg-white">
                      ...
                      <td className="px-4 py-3 text-slate-500 font-mono text-xs">
                        {task.targetDate
                          ? dayjs(task.targetDate).format('MMM D, YYYY')
                          : 'TBD'}
                      </td>
      ...
      >
        <div className="mt-4 mb-2 text-sm text-slate-600">
          Please provide a reason for rejecting this training plan. This will be sent back to the
          enterprise.
        </div>
        <TextArea
          rows={4}
