...
for (let i = 0; i < lines.length; i++) {
  ...
  try {
    ...
    if (obj.content && obj.content.includes('Edit Profile')) {
       console.log('Found Edit Profile in step', obj.step_index);
       const matches = obj.content.match(/.{0,50}Edit Profile.{0,50}/g);
