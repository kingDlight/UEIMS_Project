...
public interface EnterpriseAssignmentRepository extends JpaRepository<EnterpriseAssignment, UUID> {
    ...
    List<EnterpriseAssignment> findByEnterprise_EnterpriseIdAndSemester_SemesterId(UUID enterpriseId, UUID semesterId);
    ...
    // UC-45: Lọc danh sách phân công theo doanh nghiệp và học kỳ đang ACTIVE.
    // Hiển thị tất cả SV đã được phân công thực tế cho DN trong kỳ ACTIVE —
    // bao gồm cả SV đã OJT/COMPLETED (để HR xem evaluation, final report).
