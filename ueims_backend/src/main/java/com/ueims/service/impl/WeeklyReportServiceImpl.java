...
public class WeeklyReportServiceImpl implements WeeklyReportService {
    ...
    public WeeklyReport save(WeeklyReport entity) {
        ...
        if (entity.getWeekNumber() != (int) currentWeek) {
            // Check if Training Manager explicitly overrides (Not implemented here, but block by default)
            throw new AppException(ErrorCode.APPLICATION_DEADLINE_EXPIRED); // reusing code or throwing general constraint
        }
    ...
    public WeeklyReport updateReport(UUID id, WeeklyReportRequest request) {
        ...
        if ("APPROVED".equals(status) || "PENDING_REVIEW".equals(status) || "REVIEWED".equals(status)) {
            throw new AppException(ErrorCode.APPLICATION_STATUS_CHANGED); // Using as generic status lock
        }
